...
$scope.choice = "greater-equal";

$scope.array = [ "less", "greater","greater-equal","less-equal"];

$scope.map= {'greater-equal': '>=' ,'greater': '>','less-equal': '<=','less':'<' };
$scope.query="";
$scope.userData="";
...
if($scope.userData==""){
 $scope.flag="No data available";
    ...
    {
      for (var i = 1; i <= $scope.userData.items.length; i++) {
  
  
    var side = $scope.userData.items[i];
  
    $scope.meals.push(side);
  }
    }
...
if($scope.stars!="" && $scope.language!=""){
  
    return $http.get("https://api.github.com/search/repositories?q=stars:"+$scope.map[$scope.choice]+$scope.stars+" language:"+$scope.language+"&sort=stars&order=desc")
...
if($scope.stars!="" && $scope.language==""){
    ...
    if($scope.stars=="" && $scope.language!=""){
    return $http.get("https://api.github.com/search/repositories?q=language:"+$scope.language+"&sort=stars&order=desc")
    .success(function(response) {
        $scope.userData = response;
      $scope.add();
         })
